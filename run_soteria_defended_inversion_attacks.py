...
def calculate_batch_inversion_performance_parallelized(dataset, network_layout, training_epochs,
                                                       training_batch_size, reconstruction_batch_sizes, prunning_rate,
                                                       tolerance_map, n_samples, config, first_cpu, max_n_cpus,
                                                       metadata_path, device, soteria):
    """
    ...
    :param prunning_rate: (list) A list of the prunning rates to try out.
    ...
    collected_data = np.zeros((len(training_epochs), len(reconstruction_batch_sizes), len(prunning_rate), 3 + len(dataset.train_features), 5))
    ...
    timer = Timer(len(training_epochs) * len(reconstruction_batch_sizes)* len(prunning_rate))
    ...
    for i, training_epoch in enumerate(training_epochs):
        ...
        for j, reconstruction_batch_size in enumerate(reconstruction_batch_sizes):
            ...
            for l, scale in enumerate(pruning_rate):
                ...
                for sample_group in sample_grouping:
                    ...
                    prompts = [f'taskset -c {cpu + first_cpu} python single_inversion_fedsgd.py --metadata_path {curr_metadata_path} --batch_size {reconstruction_batch_size} --sample {s} --random_seed {random_seeds[s]} --soteria --pruning {scale} --device {device}' for cpu, s in enumerate(sample_group)]
...
def main(args):
    ...
    specific_file_path = base_path + f'/DP_data_all_{args.experiment}_{args.dataset}_{args.n_samples}_{file_name_post_selection}_{file_name_max_iterations}_{reconstruction_batch_sizes[-1]}_{prunning_rate[-1]}_{tol}_{args.random_seed}.npy'
    if os.path.isfile(specific_file_path) and not args.force:
        ...
        inversion_data = calculate_batch_inversion_performance_parallelized(
            ...
            prunning_rate=prunning_rateprunning_rate,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--soteria', action='store_true', help='Use soteria defense')
